...
#make the phyloseq object to use
...
#extract the data so you can access it at will

richness_vals_f <- f_richness$data
Shannon_vals_f <-  f_Shannon$data
Simpson_vals_f <- f_Simpson$data

#double check with wilcoxon rank sum test
wilcox.test(richness_vals_f,formula = Value~Group)
wilcox.test(Shannon_vals_f,formula = Value~Group)
wilcox.test(Simpson_vals_f,formula = Value~Group)
...
alpha_richnes <- z(f_richness,col=childish,chart="box") + geom_jitter(aes(color=Group)) + labs(titile ="Alpha diversity of Faeces samples",y = "Richness (q=0)") + theme(legend.position = "none",title = element_text(face = "italic")) + ylim(c(0,40))

alpha_shannon <- z(f_Shannon,posthoc=TRUE,threshold=0.05,col=childish,chart="box") + geom_jitter(aes(color=Group)) + labs(titile ="Alpha diversity of Faeces samples",y = "Shannon diversity (q=1)") + theme(legend.position = "none",title = element_text(face = "italic")) + ylim(c(0,5))

alpha_simpson <- z(f_Simpson,posthoc=TRUE,threshold=0.05,col=childish,chart="box") + geom_jitter(aes(color=Group)) + labs(titile ="Alpha diversity of Faeces samples",y = "Simpson diversity (q=2)") + theme(legend.position = "none",title = element_text(face = "italic")) + ylim(c(0,5))


alpha_final <- alpha_richnes + alpha_shannon + alpha_simpson + plot_annotation(tag_levels = "A",title = "Alpha deversities",subtitle = "Genus level") & theme(title = element_text(size=20),axis.text = element_text(size=15))
...
wilcox_test(richness_vals_f,formula = Value~Group)
wilcox_test(Shannon_vals_f,formula = Value~Group)
wilcox_test(Simpson_vals_f,formula = Value~Group)
...
alpha_richnes <- z(f_richness,col=childish,chart="box") + geom_jitter(aes(color=Group)) + labs(titile ="Alpha diversity of Faeces samples",y = "Richness (q=0)") + theme(legend.position = "none",title = element_text(face = "italic"))

alpha_shannon <- z(f_Shannon,col=childish,chart="box") + geom_jitter(aes(color=Group)) + labs(titile ="Alpha diversity of Faeces samples",y = "Shannon diversity (q=1)") + theme(legend.position = "none",title = element_text(face = "italic")) 

alpha_simpson <- z(f_Simpson,posthoc=TRUE,threshold=0.05,col=childish,chart="box") + geom_jitter(aes(color=Group)) + labs(titile ="Alpha diversity of Faeces samples",y = "Simpson diversity (q=2)") + theme(legend.position = "none",title = element_text(face = "italic"))


alpha_final <- alpha_richnes + alpha_shannon + alpha_simpson + plot_annotation(tag_levels = "A",title = "Alpha deversities",subtitle = "ASV level")
```
Abundances
```{r}
...
#tiff("metacoder_plot1_BioMar.tiff", units="in", width=10, height=5, res=300)
...
ordipoo_bray <- ordinate(physeq_faeces,"PCoA","bray")
ordipoo_jaccard <- ordinate(physeq_faeces,"PCoA","jaccard")
...
adonis(t(as.data.frame(physeq_faeces@otu_table))~Batch,data=as.data.frame(as.matrix(physeq_faeces@sam_data)),method="bray")
adonis(t(as.data.frame(physeq_faeces@otu_table))~Fish_type,data=as.data.frame(as.matrix(physeq_faeces@sam_data)),method="bray")
adonis(t(as.data.frame(physeq_faeces@otu_table))~Water.condition,data=as.data.frame(as.matrix(physeq_faeces@sam_data)),method="bray")

#add the significant taxa to the picture and do adonis again

physeq_faeces@sam_data$Ceto <- as.numeric(t(physeq_faeces@otu_table["ASV_1"]))
physeq_faeces@sam_data$Aero <- t(physeq_faeces@otu_table["ASV_3"])
physeq_faeces@sam_data$Vibrio <- t(physeq_faeces@otu_table["ASV_24"])
physeq_faeces@sam_data$Pseudo <- t(physeq_faeces@otu_table["ASV_13"])

#contributions of individual taxa
p <- t(as.data.frame(physeq_faeces@otu_table))
d <- as.data.frame(as.matrix(physeq_faeces@sam_data))
d$Ceto <- as.numeric(d$Ceto)
d$Aero <- as.numeric(d$Aero)
d$Vibrio <- as.numeric(d$Vibrio)
d$Pseudo <- as.numeric(d$Pseudo)
#How much of the observed difference can be attributed to each genus.
adonis(p~Ceto,data=d,method="jaccard")
adonis(p~Aero,data=d,method="jaccard")
adonis(p~Vibrio,data=d,method="jaccard")
adonis(p~Pseudo,data=d,method="jaccard")
